...
{
	nk_draw_command *cmd = nk_draw_list_push_command(list, nk_null_rect, list->config.tex_null.texture);
	if (cmd)
	{
		if (!list->cmd_count) {
        	nk_draw_list_push_command(list, nk_null_rect, list->config.tex_null.texture);
		} else {
			struct nk_draw_command *prev = nk_draw_list_command_last(list);
