...
nk_draw_checkbox(struct nk_command_buffer *out,
    ...
    const struct nk_user_font *font,int rtl)
{
    ...
    nk_widget_text(out, *label, string, len, &text, NK_TEXT_START(rtl), font);
...
nk_do_toggle(nk_flags *state,
    ...
    const struct nk_user_font *font, nk_flags widget_alignment, int rtl)
{
    int was_active;
	...
	if (rtl) {
		select.x = r.w - select.w;
		label.w -= style->spacing;
		label.x = select.x - label.w;
	}
    label.y = select.y;
    label.h = select.w;
    ...
    *active = nk_toggle_behavior(in, bounds, state, *active);
    ...
    if (type == NK_TOGGLE_CHECK) {
        nk_draw_checkbox(out, *state, style, *active, &label, &select, &cursor, str, len, font,rtl);
    } else {
        nk_draw_option(out, *state, style, *active, &label, &select, &cursor, str, len, font,rtl);
    }
    if (style->draw_end)
...
nk_check_text(struct nk_context *ctx, const char *text, int len, nk_bool active)
{
    ...
    nk_do_toggle(&ctx->last_widget_state, &win->buffer, bounds, &active,
        text, len, NK_TOGGLE_CHECK, &style->checkbox, in, style->font, NK_WIDGET_LEFT, ctx->rtl);
    return active;
}
NK_API nk_bool
...
nk_option_text(struct nk_context *ctx, const char *text, int len, nk_bool is_active)
{
    ...
    nk_do_toggle(&ctx->last_widget_state, &win->buffer, bounds, &is_active,
        text, len, NK_TOGGLE_OPTION, &style->option, in, style->font, NK_WIDGET_LEFT, ctx->rtl);
    return is_active;
}
NK_API nk_bool
